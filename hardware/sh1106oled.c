...
void OLED_ShowTime(uint8_t clock_hour, uint8_t clock_min, uint8_t clock_sec)
{
    ...
    sh1106_fill_rect(38, 24, 48, 16, 0);
    // 时十位
    OLED_ShowNum(38,   24, hour_tens, 8);
    // 时个位
    OLED_ShowNum(46,  24, hour_units, 8);
    // 冒号（需要单独制作冒号位图）
    OLED_ShowColon(54, 24);  // 自定义函数
    // 分十位
    OLED_ShowNum(58,  24, min_tens, 8);
    // 分个位
    OLED_ShowNum(66,  24, min_units, 8);
    // 秒十位
    OLED_ShowNum(74,  29, sec_tens , 5);
    // 秒个位
    OLED_ShowNum(80,  29,sec_units, 5);
    // 刷新显示（draw_bitmap 内部已调用 refresh）
	sh1106_refresh_gram();
}
void Show_year_mouth_day(uint16_t NPTyears, uint8_t NPTmouths, uint8_t NPTdays)
{
    uint8_t year01   = NPTyears / 100;
    uint8_t year02   = NPTyears % 100;
    uint8_t year01_units   = year01 % 10;
    uint8_t year01_tens    = year01 / 10;
    uint8_t year02_units   = year02 % 10;
    uint8_t year02_tens    = year02 / 10;
    uint8_t month_units   = NPTmouths % 10;
    uint8_t month_tens    = NPTmouths / 10;
    uint8_t day_units  = NPTdays % 10;
    uint8_t day_tens   = NPTdays / 10;
    sh1106_fill_rect(10, 6, 112, 16, 0);
    OLED_ShowNum(10, 6, year01_tens, 8);    
    OLED_ShowNum(18, 6, year01_units, 8);
    OLED_ShowNum(26, 6, year02_tens, 8);
    OLED_ShowNum(34, 6, year02_units, 8);
    sh1106_draw_bitmap(42, 6, bitmap_years, 16, 16);
    OLED_ShowNum(58, 6, month_tens, 8);
    OLED_ShowNum(66, 6, month_units, 8);
    sh1106_draw_bitmap(74, 6, bitmap_mouths, 16, 16);
    OLED_ShowNum(90, 6, day_tens, 8);
    OLED_ShowNum(98, 6, day_units, 8);
    sh1106_draw_bitmap(106, 6, bitmap_days, 16, 16);
    sh1106_refresh_gram();
}
...
void OLED_ShowChar(uint8_t x, uint8_t y, char ch)
{
    ...
    // 检查字符是否在可打印范围内（ASCII 32 ~ 126）
    if (ch < 32 || ch > 126)
    {
        index = 0;   // 不可打印字符，显示为空格
    }
    else
    {
        index = ch - 32; // 空格对应索引0
    ...
    // 使用8×16位图绘制字符
    sh1106_draw_bitmap(x, y, ascii_8x16[index], 8, 16);
}
